...
export default class NotificationService {
  ...
  public async findNotificationsOfUser(data: any): Promise<{
    notifications: Notification[];
    total: number;
    totalNotRead: number;
  }> {
    const notifications: Notification[] =
      await this.notificationRepository.findAndSort(
        data.filter,
        data.skip,
        data.take
      );
  ...
  public async createNotification(
    ...
    if (isEmpty(data))
      throw new HttpException(409, "notification data is empty");
  ...
  public async updateNotification(
    ...
    if (!checkNotificationId)
      throw new HttpException(409, "notification doesn't exist");
