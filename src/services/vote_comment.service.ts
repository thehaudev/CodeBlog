...
export default class Vote_CommentService {
    ...
    public async createVoteComment(data: any): Promise<Vote_comment> {
        if (isEmpty(data)) throw new HttpException(409, "data is empty")

        const checkCommentId = await this.comments.findById(data.commentId)
    ...
    public async updateVoteComment(data: any): Promise<Vote_comment | null> {
        if (isEmpty(data)) throw new HttpException(409, "data is empty")

        const checkVote = await this.vote_commentRepository.findOne({ userId: data.userId, commentId: data.commentId })
        if (!checkVote) throw new HttpException(409, "vote doesn't exist")

        return await this.vote_commentRepository.update(checkVote._id + "", { type: data.type })
    }
